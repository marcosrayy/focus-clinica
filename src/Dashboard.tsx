...
function Toggle({ value, onChange }: { value: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      style={{ width: 44, height: 24, background: value ? "#0d9488" : "#4b5563", transition: "background 200ms" }}
    >
      <span
        className="absolute top-0.5 rounded-full bg-white shadow-sm"
        style={{ width: 20, height: 20, transform: value ? "translateX(22px)" : "translateX(2px)", transition: "transform 200ms" }}
      />
    </button>
  );
}
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 py-5">
          ...
          <div>
            <span className="font-bold text-white" style={{ fontFamily: "'Outfit', sans-serif", fontSize: "1.1rem", letterSpacing: "-0.01em" }}>Clin Focus</span>
...
function Header({ onMenuClick }: { onMenuClick: () => void }) {
  ...
  return (
    <header
      ...
      style={{ height: 64, background: t.headerBg, borderBottom: `1px solid ${t.border}` }}
    >
      <div className="flex items-center gap-3">
        {/* Hamburger */}
        <button
          className="md:hidden w-9 h-9 flex items-center justify-center rounded-xl cursor-pointer"
          style={{ background: t.mutedBg, color: t.textMuted }}
          onClick={onMenuClick}
        >
          <IconMenu />
        </button>
      ...
      <div className="flex items-center gap-2 md:gap-3">
        ...
        <span className="hidden md:block text-xs font-medium px-3 py-1.5 rounded-lg" style={{ background: "#f0fdf9", color: "#0d9488" }}>
          Qui, 10 set. 2026
        </span>
        ...
        <button
          ...
          style={{ background: t.mutedBg, border: `1px solid ${t.borderStrong}`, color: t.textMuted }}
          onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = dark ? "#1e2d42" : "#f0fdf9"; (e.currentTarget as HTMLElement).style.color = "#0d9488"; }}
          onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = t.mutedBg; (e.currentTarget as HTMLElement).style.color = t.textMuted; }}
        >
        ...
        <button
          ...
          style={{ background: t.mutedBg, border: `1px solid ${t.borderStrong}`, color: t.textMuted }}
          onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = "#f0fdf9"; (e.currentTarget as HTMLElement).style.color = "#0d9488"; }}
          onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = t.mutedBg; (e.currentTarget as HTMLElement).style.color = t.textMuted; }}
        >
          <IconBell />
        ...
        >
          ...
          <div className="hidden md:block">
            <p className="text-xs font-semibold leading-tight" style={{ color: t.textPrimary }}>Marcos Ray</p>
            <p style={{ fontSize: "10px", color: t.textFaint }}>Administrador</p>
          </div>
          <span style={{ color: t.textFaintest }} className="hidden md:block"><IconChevronDown /></span>
        </div>
      </div>
    </header>
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between">
        ...
        {trend && (
          <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ background: "#dcfce7", color: "#15803d" }}>{trend}</span>
        )}
      </div>
      <div>
  ...
  return (
    <div className="flex flex-col gap-6 md:gap-7">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        ...
        <KpiCard label="Na fila agora" value="3" sublabel="Aguardando atendimento" icon={<IconClock />} accentColor="#f59e0b" />
        <KpiCard label="Agendamentos" value="1" sublabel="Consulta pendente" icon={<IconCalendarCheck />} accentColor="#3b82f6" />
        <KpiCard label="Próximo" value="09:30" sublabel="Maria Silva · Clínica Geral" icon={<IconClock />} accentColor="#8b5cf6" />
      </div>

      {/* Calendar + Notifications — stack on mobile */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6" style={{ minHeight: 0 }}>
        <CalendarWidget />
        <NotificationsPanel />
      </div>
    </div>
